...
mat rcpp_d_k_sese_l(const mat &M, const mat &N, const mat &P, const mat &Q, const double l, const double s, const double m, const bool equal_matrices) {
  mat d_K_se_l, K_se, res;
  d_K_se_l = rcpp_d_k_se_l(M, N, l, s, equal_matrices);
...
mat rcpp_d_k_sese_m(const mat &M, const mat &N, const mat &P, const mat &Q, const double l, const double s, const double m, const bool equal_matrices) {
  mat d_K_se_m, K_se, res;
  d_K_se_m = rcpp_d_k_se_l(P, Q, m, 1, equal_matrices);
  K_se = rcpp_k_se(M, N, l, s, equal_matrices);
  res = d_K_se_m % K_se;
  return res;
}
...
// [[Rcpp::export]]
double rcpp_step_k_sese_svi(const mat &M, const mat &N, const mat &P, const mat &Q,
  ...
  const mat &inv_K_mm, const mat &K_mn, const vec &y, const vec &mu, const mat &S, const int par_id) {
    ...
    if (par_id == 1) {
      ...
      d_K_mm = rcpp_d_k_sese_l(M, N, P, Q, l, s, m, 1);
      d_K_mn = rcpp_d_k_sese_l(M, N, P, Q, l, s, m, 0);
      for (size_t i = 0; i < N.n_rows; i++) {
        diag_d_K_nn(i) = d_k_se_l(N.row(i).t(), N.row(i).t(), l, s) * k_se(Q.row(i).t(), Q.row(i).t(), m, 1);
      }
    } else if (par_id == 2) {

      d_K_mm = rcpp_d_k_sese_s(M, N, P, Q, l, s, m, 1);
      d_K_mn = rcpp_d_k_sese_s(M, N, P, Q, l, s, m, 0);
      for (size_t i = 0; i < N.n_rows; i++) {
        diag_d_K_nn(i) = d_k_se_s(N.row(i).t(), N.row(i).t(), l, s) * k_se(Q.row(i).t(), Q.row(i).t(), m, 1);
      }
    } else {

      d_K_mm = rcpp_d_k_sese_m(M, N, P, Q, l, s, m, 1);
      d_K_mn = rcpp_d_k_sese_m(M, N, P, Q, l, s, m, 0);
      for (size_t i = 0; i < N.n_rows; i++) {
        diag_d_K_nn(i) = k_se(N.row(i).t(), N.row(i).t(), l, s) * d_k_se_l(Q.row(i).t(), Q.row(i).t(), m, 1);
      }
    }
